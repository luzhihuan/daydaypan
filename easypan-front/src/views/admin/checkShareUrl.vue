      ...
      >
        <template #fileName="{index,row}">
          <div class="file-item" @mouseenter="showOp(row)" @mouseleave="cancelShowOp(row)">
            ...
            <span class="op">
                  <span class="iconfont icon-del" @click="delFile(row)">退回</span>
                  <span class="iconfont icon-import" @click="passFile(row)">通过</span>
              </span>
          </div>
        </template>
...
const loadAllFileList = async () => {
  ...
  let result = await proxy.Request({
    url: proxy.Api.loadFileJudgeList,
...
const delFile = async (row) => {
  proxy.Confirm({
    message: `你要不通过${row.fileName}这个文件的分享吗`,
    okfun: async () => {
      let result = await proxy.Request({
        url: proxy.Api.handleFileJudge,
        params: {
          opType: 'fail',
          shareId: row.shareId,
        }
...
//返回
const passFile = async (row) => {
  proxy.Confirm({
    message: `你要通过${row.fileName}这个文件的分享吗`,
    okfun: async () => {
      let result = await proxy.Request({
        url: proxy.Api.handleFileJudge,
        params: {
